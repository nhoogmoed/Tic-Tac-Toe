...
def getMoveFromAI(agentPiece: Piece, board: Board) -> Move:
  ...
  # Step One: If agent can win in 1 move, agent makes move

  for run in AllRuns:

    indices = list(getRunIndices(run))

    pieces = [board[row][col] for row, col in indices]

    if pieces.count(Piece.Empty) == 1 and pieces.count(agentPiece) == 2:

      i = pieces.index(Piece.Empty)

      pos = indices[i]

      return Move(agentPiece, *pos)

  # step two: If agent can lose in the next move, block that move
  ...
  for run in AllRuns:
    ...
    if pieces.count(Piece.Empty) == 1 and pieces.count(otherPiece) == 2:
  ...
  for x, y in ((1, 1), (0, 0), (0, 2), (2, 0), (2, 2), (1, 0), (1, 2), (0, 1),
